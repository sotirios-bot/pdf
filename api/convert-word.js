// Serverless conversion endpoint: PDF (raw request body) -> Word (.docx).
//
// Deployed on Vercel as /api/convert-word. The browser POSTs the raw PDF bytes
// with Content-Type: application/pdf. We use PDF.js to extract text with
// positions, rebuild reading-order lines per page, and write them as paragraphs
// into a Word document with the `docx` library (one page break per PDF page).
//
// Handles digital/text-based PDFs. Scanned (image-only) PDFs have no text layer
// and return a clear error. TODO (later): layout-aware conversion and OCR.
...
// Core conversion, exported for testing. Returns a .docx Buffer.
// Throws Error("NO_TEXT") when the PDF has no extractable text layer.
export async function convertPdfToDocx(buffer) {
  ...
  for (let p = 1; p <= doc.numPages; p++) {
    ...
    if (p > 1) paragraphs.push(new Paragraph({ pageBreakBefore: true }));
...
export default async function handler(req, res) {
  ...
  try {
    const out = await convertPdfToDocx(buffer);
    res.setHeader(
      "Content-Type",
